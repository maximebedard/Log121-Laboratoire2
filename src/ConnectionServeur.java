...
public class ConnectionServeur {
	...
	/**
	 * Connexion au serveur à partir d'une adresse au format nom:[port] ou le
	 * port par défaut est égal à 10000
	 * 
	 * @param addr
	 *            adresse du serveur
	 * @throws IOException
	 *             Exception lancé lors des erreurs de connexion
	 */
	public void connect(String addr) throws IOException {
		...
		if (addrParts.length == 0 || addrParts.length > 2)
			throw new MalformedURLException(
					"L'adresse contient plus d'un ':' pour spécifier le port.");
		...
		if (addrParts.length > 1) {
			try {
				...
				throw new MalformedURLException(
						"Le numéro de port n'est pas un entier");
			}
		}
	...
	/**
	 * Intéroge le serveur dans le but d'obtenir la définition d'une forme sous
	 * la forme d'une chaîne de caractère.
	 * 
